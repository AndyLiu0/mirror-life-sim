...
static func instantiate(position):
	var new: MirrorBacteria = scene.instantiate()
...
func _ready():
	...
	connect("mouse_entered", func(): 
		Nametag.instantiate(self, "Mirror Bacteria", 50))
...
func _process(delta):
	if die != 0:
		...
		if die >= 1:
			get_parent().mirror_bacteria_count -= 1
			queue_free()
		return
	...
	if Globals.IN_RECT.has_point(Vector2(position.x, position.z)):
		leave = randf() < 1 if get_parent().mirror_bacteria_count > get_parent().mirror_bacteria_level else 0.2
	else:
